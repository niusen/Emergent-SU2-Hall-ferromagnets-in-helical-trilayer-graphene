...
filenm="FBZ_paircorrel_theta1.44_waa"+num2str(waa,'%.1f')+"_epsilon4_Np24_sec";
...
for ck=1:length(K_id_set)
    ...
    for cs=1:n_keep_set(ck)
        ...
        for cq=1:size(q_FBZ,1)
            k1234=k1234_set{cq};
            t1=ob_terms_set{cq};
            for dd=1:size(k1234,1)
                if k1234(dd,2)==k1234(dd,3)
                    kk=k1234(dd,1);
                    t1(dd,:)=t1(dd,:)-nk(kk,:);
                end
            end
            ...
            t2=V1234_set{cq};
            tt=contracttensors(t2,3,1,t1,2,1);
            tt=permute(tt,[3,1,2]);
            Sq_all(cq,step,:,:)=tt(cs,:,:)/(Np^2);
...
Sq_average_subtract=Sq_average;
pos=find((q_FBZ(:,1).^2+q_FBZ(:,2).^2)<1e-10);
Sq_average_subtract(pos,:,nK+1,nK+1)=Sq_average_subtract(pos,:,nK+1,nK+1)*0;
value_range=[min(real(Sq_average_subtract(:))),max(real(Sq_average_subtract(:)))];
...
%save("Sq_36_24"+"_waa"+num2str(waa)+".mat","corners","Sq_values","Sq_points","g1","g3");
...
for ca=-nK:nK
    for cb=-nK:nK
        for c1=1:size(q_FBZ,1)
            ...
            gr=get_gr(Sq_average,K_set,coord);
...
save("gr_36_24"+"_waa"+num2str(waa)+".mat","gr_values","gr_points","ax","ay","coord0","Dx","Dy");
